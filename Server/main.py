...
class Server:
                    ...
                    if msg:
                        ...
                        if client.online:
                            if msg == "log_out": # when client logs out
                                ...
                                self.file_send_when_accessed(path, client_id, size, db_conn)
    ...
    def file_send(self, client_id, path, size):
        client = self.CLIENTS[client_id]
        if client:
            file = open(f'users/{path}', "rb")
            data = file.read()
    ...
    def file_send_when_accessed(self, path, who_updated_client_id, size, db_conn):
        ...
        for client_id in guests_client_ids:
            ...
            if (not client_id == who_updated_client_id) and (not client_id == None): 
                self.file_send(client_id, path, size)
